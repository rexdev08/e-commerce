...
function renderProduct(obj) {
  return `<div class="item" id="${obj.id}">
            ...
            <button data-key="${obj.id}" id="product-${obj.id}" class="item__button">anadir al carrito</button>
          </div>`
}
...
const renderItem = (obj) => {
  ...
  <div class="item__split item__split--right">
    ...
    <div class="item__buttons">
      ...
      <span data-identifier="${obj.id}" id="quantity" class="item__quantity">${obj.quantity}</span>
...
function updateTotal() {
  ...
  const total = shoppingCart.map((obj) => {
...
const itemsTotal = shoppingCart.map((obj) => {
  return  obj.quantity
}).reduce((value, nextValue) => {
  return value + nextValue
}, 0)
  ...
  $headerCounter.innerText > 0 ? $headerCounter.classList.add("header__counter--active"):$headerCounter.classList.remove("header__counter--active")
};
...
function filterProducts(element = "sofa") {
  ...
  $productsContainer.innerHTML = selection.map(product => renderProduct(product)).join("")
    ...
    boton.addEventListener("click", () => {
      ...
      btnsRemove.forEach(btn => {
        btn.addEventListener("click", () => {
          ...
          shoppingCart = shoppingCart.filter(obj => obj.id != btn.parentElement.parentElement.dataset.id)
          storage.find(obj => obj.id == btn.parentElement.parentElement.dataset.id).quantity = 1
      ...
      btnMinus.forEach(btn => {
        btn.addEventListener("click", () => {
          const productId = btn.parentElement.parentElement.parentElement.dataset.id
          const qty = $modalItems.querySelector(`[data-identifier="${productId}"]`)
          const product = storage.filter(obj => obj.id === parseInt(productId))
